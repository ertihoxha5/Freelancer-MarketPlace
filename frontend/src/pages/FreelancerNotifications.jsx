...
function NotificationIcon({ type, isRead }) {
  ...
  return (
    <div
      className={`${base} ${isRead ? "bg-slate-100 text-slate-400" : "bg-emerald-100 text-emerald-600"}`}
    >
      🔔
    </div>
  );
...
export default function FreelancerNotifications() {
  ...
  // Njofto Header-in për badge update
  useEffect(() => {
            ...
            <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Project updates, messages, and platform activity.
                </p>
              </div>
...
function EmptyState({ filter }) {
  const messages = {
    all: {
      ...
      desc: "Project and message activity will appear here.",
    },
    unread: {
      icon: "✅",
      title: "All caught up!",
    ...
    system: {
      ...
      desc: "Project updates and platform events will appear here.",
    },
    message: {
      icon: "💬",
      title: "No message notifications",
      desc: "New messages from clients will appear here.",
    },
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-3xl border border-dashed border-slate-200 bg-slate-50 py-20 text-center">
      ...
      <Link
        ...
        className="mt-8 rounded-2xl bg-emerald-700 px-6 py-2.5 text-sm font-semibold text-white hover:bg-emerald-800 transition-colors"
      >
        Go to Dashboard
      </Link>
    </div>
