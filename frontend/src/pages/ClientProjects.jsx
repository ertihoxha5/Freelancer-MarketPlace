...
export default function ClientProjects() {
  ...
  const filteredProjects = useMemo(() => {
    ...
    return projects.filter((project) => {
      ...
      const matchesStatus =
        statusFilter === "any" || project.pStatus === statusFilter;
      const deadline = project.deadline ? new Date(project.deadline) : null;
      const fromOk =
        deadlineFrom === "" || (deadline && deadline >= new Date(deadlineFrom));
      const toOk =
        ...
        <div className="flex h-full min-h-0 flex-col overflow-hidden border-t border-slate-200 bg-white lg:flex-row">
          ...
          <section className="min-h-full min-w-0 flex-1 overflow-auto p-6 sm:p-8">
            <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="mt-2 text-slate-600 max-w-2xl">
                  Filter, review, and manage the projects you posted.
                </p>
              </div>
              <Link
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={() => exportJSON(filteredProjects, "my-projects")}
                className="rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-700 hover:bg-slate-50"
              >
                Export JSON
              </button>
            ...
            <form onSubmit={handleEdit} className="space-y-5 p-6">
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setEditForm({ ...editForm, pDesc: e.target.value })
                  }
                  rows={4}
                  className="w-full rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setEditForm({ ...editForm, pStatus: e.target.value })
                  }
                  className="w-full rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                >
                  <option value="pending">Pending</option>
