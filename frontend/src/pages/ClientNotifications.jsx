            ...
            <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div>
                ...
                <h1 className="mt-2 text-3xl font-semibold text-slate-900 flex items-center gap-3">
                  Notifications
                  {unreadCount > 0 && (
                    <span className="inline-flex items-center justify-center rounded-full bg-[#1a3c2e] px-2.5 py-0.5 text-xs font-bold text-white">
                      {unreadCount}
                    </span>
                  )}
                </h1>
                <p className="mt-1 text-sm text-slate-500">
                  Important updates about your projects and platform activity.
                </p>
              </div>
                ...
                const label =
                  f === "all"
                    ...
                    : f === "unread"
                      ? `Unread (${unreadCount})`
                      : f === "system"
                        ? `System (${notifications.filter((n) => n.types === "system").length})`
                        : `Messages (${notifications.filter((n) => n.types === "message").length})`;
...
function EmptyState({ filter }) {
  const messages = {
    ...
    unread: {
      ...
      desc: "You've read everything. Great job staying on top of things!",
    },
    system: {
      icon: "🔔",
      title: "No system notifications",
      desc: "System alerts about your projects and account will appear here.",
    },
    message: {
      icon: "💬",
      title: "No message notifications",
      desc: "Notifications from freelancer messages will appear here.",
    },
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-3xl border border-dashed border-slate-200 bg-slate-50 py-20 text-center">
      ...
      <Link
        to="/client/dashboard"
        className="mt-8 rounded-2xl bg-[#1a3c2e] px-6 py-2.5 text-sm font-semibold text-white hover:bg-[#2a5c46] transition-colors"
      >
        Go to Dashboard
      </Link>
    </div>
