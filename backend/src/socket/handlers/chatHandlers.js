...
export function registerChatHandlers({ io, socket, presenceState }) {
    socket.on('conversation:join', async (payload = {}, ack) => {
        try {
            ...
            const conversation = await chatRepository.getConversationByIdForUser(conversationID, socket.user.id);
            if (!conversation) {
    ...
    socket.on('message:send', async (payload = {}, ack) => {
        try {
            ...
            const participants = await chatRepository.getConversationParticipants(message.conversationID);
    ...
    socket.on('message:read', async (payload = {}, ack) => {
        try {
            ...
            io.to(conversationRoom(Number(payload.conversationID))).emit('message:read', {
